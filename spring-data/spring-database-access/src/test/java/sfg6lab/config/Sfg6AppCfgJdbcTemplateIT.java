...
class Sfg6AppCfgJdbcTemplateIT {
    ...
    @Nested
    @DisplayName("Test SimpleJdbcInsert")
    class SimpleJdbcInsertTest {
        ...
        void setUp() {
            ...
            deleteLikes(likerFk, likeeFk);
        }

        private void deleteLikes(int likerFk, int likeeFk) {
            namedParamJdbcTemplate.update(
                    "DELETE FROM likes " +
                            "WHERE liker_fk = :likerFk AND likee_fk = :likeeFk",
                    Map.ofEntries(
                            entry("likerFk", likerFk),
                            entry("likeeFk", likeeFk)));
        }

        private Integer countLikes(int likerFk, int likeeFk) {
        ...
        @Test
        void should_Be_Able_To_Add_One_New_Row_Without_Specifing_Col_Names_In_Sql() {

            // Given
            assertThat(countLikes(likerFk, likeeFk)).isZero();

            // When
            jdbcInsertLikes.execute(Map.of(
                    "liker_fk", likerFk, "likee_fk", likeeFk));

            // Then
            assertThat(countLikes(likerFk, likeeFk)).isOne();
        }
